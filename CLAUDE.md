...
- **Undo** — visible when at batter step with prior at-bat. Calls `undoLastAtBat()` which pre-fills wizard with the last at-bat, then deletes it (and replays the half-inning). Only the most recent at-bat is undoable.
- **`deleteAtBat`** — replays the current half-inning from scratch after deletion to recompute `liveRunners` and `outs`. Also rewinds lineup position for non-sub at-bats. If outs drop below 3 and we were showing `inning_end`, transitions step back to `batter`.
